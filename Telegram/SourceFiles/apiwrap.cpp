...
void ApiWrap::markContentsRead(
	...
	if (Ghostgram::Manager::Instance().ghostMode().shouldSuppressReadHistory()) {
		return;
	}
...
void ApiWrap::sendAction(const SendAction &action) {
	if (Ghostgram::Manager::Instance().config().readOnActions()
		&& Ghostgram::Manager::Instance().ghostMode().shouldSuppressReadHistory()) {
		if (const auto last = action.history->lastServerMessage()) {
			Ghostgram::ReadUpTo(last);
		}
	}
	Ghostgram::GoOfflineAfterAction(_session);
	if (!action.options.scheduled
		&& !action.options.shortcutId
		&& !action.replaceMediaOf) {
		Ghostgram::GhostMode::ActionScope ghostgramScope;
...
void ApiWrap::sendMessage(
	...
	{
		const auto &cfg = Ghostgram::Manager::Instance().config();
		if (cfg.ghostMode()
			&& cfg.ghostSendDelay()
			&& !message.action.options.scheduled) {
			message.action.options.scheduled = base::unixtime::now()
				+ cfg.ghostTextDelaySec();
		}
	}
	const auto history = message.action.history;
	const auto peer = history->peer;
	auto &textWithTags = message.textWithTags;
	Ghostgram::ApplyOutgoingTextHooks(textWithTags);
