...
, _idleFinishTimer([=] { checkIdleFinish(); }) {
	...
	auto &ghostConfig = Ghostgram::Manager::Instance().config();
	rpl::merge(
		ghostConfig.ghostModeValue(),
		ghostConfig.hideOnlineValue(),
		ghostConfig.autoOfflineValue(),
		ghostConfig.eternalOnlineValue()
	) | rpl::skip(4) | rpl::on_next([=](bool) {
		updateOnline();
	}, _lifetime);
...
void Updates::updateOnline(crl::time lastNonIdleTime, bool gotOtherOffline) {
	const auto eternalOnline =
		Ghostgram::Manager::Instance().config().eternalOnline();
	if (!lastNonIdleTime) {
		lastNonIdleTime = Core::App().lastNonIdleTime();
	}
	if (eternalOnline) {
		lastNonIdleTime = crl::now();
	}
	crl::on_main(&session(), [=] {
	...
	bool isOnline = Core::App().hasActiveWindow(&session()) || eternalOnline;
	...
	auto ms = crl::now();
	const auto &ghostMode = Ghostgram::Manager::Instance().ghostMode();
	const auto reportedOnline = isOnline
		&& (eternalOnline
			|| (!ghostMode.shouldHideOnline()
				&& !ghostMode.shouldForceOffline()));
	if (reportedOnline != _lastWasOnline
		|| (reportedOnline && _lastSetOnline + config.onlineUpdatePeriod <= ms)
		|| (reportedOnline && gotOtherOffline)) {
		api().request(base::take(_onlineRequest)).cancel();

		_lastWasOnline = reportedOnline;
		_lastSetOnline = ms;
		if (!Core::Quitting()) {
			_onlineRequest = api().request(MTPaccount_UpdateStatus(
				MTP_bool(!reportedOnline)
			)).send();
		} else {
			_onlineRequest = api().request(MTPaccount_UpdateStatus(
				MTP_bool(!reportedOnline)
		...
		self->updateLastseen(Data::LastseenStatus::OnlineTill(
			base::unixtime::now() + (reportedOnline ? onlineFor : -1)));
		...
		_lastSetOnline = ms;
	} else if (isOnline && reportedOnline) {
