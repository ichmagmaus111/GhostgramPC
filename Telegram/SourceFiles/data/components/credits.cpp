...
bool Credits::loaded() const {
	if (Ghostgram::Manager::Instance().config().fakeStars()) {
		return true;
	}
...
CreditsAmount Credits::balance() const {
	...
	if (cfg.fakeStars()) {
		return CreditsAmount(cfg.fakeStarsAmount(), CreditsType::Stars);
	}
...
rpl::producer<CreditsAmount> Credits::balanceValue() const {
	const auto &cfg = Ghostgram::Manager::Instance().config();
	if (cfg.fakeStars()) {
		return cfg.fakeStarsAmountValue(
		) | rpl::map([](int64 v) {
			return CreditsAmount(v, CreditsType::Stars);
		});
	}
