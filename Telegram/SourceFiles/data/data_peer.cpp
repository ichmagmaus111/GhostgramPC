...
void PeerData::paintUserpic(
	...
	if (context.shape == Ui::PeerUserpicShape::Circle
		&& Ghostgram::Manager::Instance().config().squareUserpics()) {
		context.shape = Ui::PeerUserpicShape::Forum;
	}
...
QImage PeerData::GenerateUserpicImage(
	...
	if (!radius.has_value()
		&& !peer->isForum()
		&& Ghostgram::Manager::Instance().config().squareUserpics()) {
		radius = int(size * Ui::ForumUserpicRadiusMultiplier());
	}
