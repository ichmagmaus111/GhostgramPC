...
void Session::updateEditedMessage(const MTPMessage &data) {
	...
	const auto previousText = existing->originalText().text;
	...
	const auto newText = existing->originalText().text;
	if (newText != previousText) {
		Ghostgram::Manager::Instance().archive()
			.notifyEdit(existing, previousText);
	}
}
...
void Session::processMessagesDeleted(
	...
	auto &archive = Ghostgram::Manager::Instance().archive();
	...
	for (const auto &messageId : data) {
		...
		if (list && i != list->end()) {
			const auto item = i->second;
			const auto history = item->history();
			if (archive.preserveOnServerDelete(item)) {
				archive.notifyDelete(item);
				requestItemViewRefresh(item);
				requestItemResize(item);
				continue;
			}
