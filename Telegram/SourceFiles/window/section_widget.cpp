...
, _controller(controller) {
	...
	::Ghostgram::Manager::Instance().chatBackground().repaintRequests(
	) | rpl::on_next([=] {
		update();
	}, lifetime());
}
...
void SectionWidget::PaintBackground(
	...
	auto &ghostBackground = ::Ghostgram::Manager::Instance().chatBackground();
	const auto paintGhost = [&] {
		if (ghostBackground.active()) {
			ghostBackground.paint(p, clip, paused);
		}
	};
