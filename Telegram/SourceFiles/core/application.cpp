...
void Application::run() {
	...
	{
		const auto &ghostConfig = Ghostgram::Manager::Instance().config();
		const auto ghostPath = ghostConfig.customFontPath();
		const auto ghostFamily = ghostConfig.customFontFamily();
		auto applied = false;
		if (!ghostPath.isEmpty() && !ghostFamily.isEmpty()) {
			if (QFontDatabase::addApplicationFont(ghostPath) >= 0) {
				style::SetCustomFont(ghostFamily);
				applied = true;
			}
		}
		if (!applied) {
