...
object_ptr<Ui::RpWidget> DetailsFiller::setupInfo() {
	...
	const auto addPeerIdLine = [&] {
		const auto bareId = int64(
			_peer->id.value & PeerId::kChatTypeMask);
		const auto idText = QString::number(bareId);
		auto line = addInfoOneLine(
			::Ghostgram::I18n::TrValue(u"ID"_q, u"ID"_q),
			rpl::single(Ui::Text::Link(idText, u"internal:copy_id"_q)),
			idText);
		line.text->overrideLinkClickHandler([=] {
			TextUtilities::SetClipboardText({ idText });
			if (const auto strong = weak.get()) {
				strong->showToast(::Ghostgram::I18n::Tr(
					u"ID copied"_q,
					u"ID скопирован"_q));
			}
		});
		if (_peer->isUser()) {
			const auto age = ::Ghostgram::EstimateAccountAge(bareId);
			if (!age.isEmpty()) {
				addInfoOneLine(
					::Ghostgram::I18n::TrValue(
						u"Account"_q,
						u"Аккаунт"_q),
					rpl::single(TextWithEntities{ age }),
					age);
			}
		}
	};
