...
void EmojiInteractions::sendAccumulatedOutgoing(
	...
	const auto suppress = Ghostgram::Manager::Instance().ghostMode().shouldSuppressTyping();
	const auto requestId = suppress
		? mtpRequestId()
...
void EmojiInteractions::playStarted(not_null<PeerData*> peer, QString emoji) {
	...
	if (!Ghostgram::Manager::Instance().ghostMode().shouldSuppressTyping()) {
		_session->api().request(MTPmessages_SetTyping(
			MTP_flags(0),
			peer->input(),
			MTPint(), // top_msg_id
