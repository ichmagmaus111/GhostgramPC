...
void AddGhostgramEditHistoryAction(
	not_null<Ui::PopupMenu*> menu,
	const ContextMenuRequest &request,
	not_null<ListWidget*> list);
void AddGhostgramReadUpToAction(
	not_null<Ui::PopupMenu*> menu,
	const ContextMenuRequest &request,
	not_null<ListWidget*> list);
...
void AddTopMessageActions(
	...
	AddGhostgramEditHistoryAction(menu, request, list);
	AddGhostgramReadUpToAction(menu, request, list);
}
...
void AddGhostgramEditHistoryAction(
	...
	const auto wasEdited = item->Has<HistoryMessageEdited>();
	const auto peerId = item->history()->peer->id.value;
	const auto messageId = item->id.bare;
	const auto hasRecord = ::Ghostgram::Manager::Instance().archive()
		.edited().hasEdits(peerId, messageId);
	if (!wasEdited && !hasRecord) {
		return;
	}
	const auto itemId = item->fullId();
	const auto controller = list->controller();
	menu->addAction(::Ghostgram::I18n::Tr(
		u"History"_q,
		u"История изменений"_q
	), crl::guard(controller, [=] {
		const auto resolved = controller->session().data().message(itemId);
		if (!resolved) {
			return;
		}
		controller->show(Box(
			::Ghostgram::EditHistoryBox,
			not_null<HistoryItem*>(resolved)));
	}), &st::menuIconEdit);
}
...
void AddGhostgramReadUpToAction(
	...
	menu->addAction(::Ghostgram::I18n::Tr(
		u"Read to here"_q,
		u"Прочитать до этого"_q
	), crl::guard(controller, [=] {
		const auto resolved = controller->session().data().message(itemId);
		if (resolved) {
			::Ghostgram::ReadUpTo(not_null<HistoryItem*>(resolved));
		}
	}), &st::menuIconMarkRead);
}
