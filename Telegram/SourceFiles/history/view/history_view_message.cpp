...
[[nodiscard]] bool IsGhostgramDeleted(not_null<const HistoryItem*> item) {
	return Ghostgram::Manager::Instance().archive().deleted().contains(
		item->history()->peer->id.value,
		item->id.bare);
}

[[nodiscard]] QString GhostgramEditPreview(not_null<const HistoryItem*> item) {
	return Ghostgram::Manager::Instance().archive().edited().previousText(
		item->history()->peer->id.value,
		item->id.bare);
}

[[nodiscard]] bool IsGhostgramEphemeral(not_null<const HistoryItem*> item) {
	return Ghostgram::Manager::Instance().mediaArchive().contains(
		item->history()->peer->id.value,
		item->id.bare);
}

void GhostgramLazySaveFile(not_null<HistoryItem*> item) {
	auto &archive = Ghostgram::Manager::Instance().mediaArchive();
	const auto peerId = item->history()->peer->id.value;
	const auto messageId = item->id.bare;
	if (!archive.contains(peerId, messageId)
		|| archive.hasFile(peerId, messageId)) {
		return;
	}
	...
	if (const auto document = media->document()) {
		...
		if (!path.isEmpty()) {
			archive.recordFileSaved(peerId, messageId, path);
		}
	}
}

void GhostgramPaintTrash(QPainter &p, QRectF rect, QColor color) {
	p.save();
	...
	auto pen = QPen(color);
	pen.setWidthF(std::max(1., s / 13.));
	pen.setCapStyle(Qt::RoundCap);
	pen.setJoinStyle(Qt::RoundJoin);
	p.setPen(pen);
	p.setBrush(Qt::NoBrush);

	const auto rimY = cy - s * 0.22;
	const auto rimHalf = s * 0.34;
	p.drawLine(QPointF(cx - rimHalf, rimY), QPointF(cx + rimHalf, rimY));

	const auto handleHalf = s * 0.12;
	const auto handleY = rimY - s * 0.12;
	p.drawLine(QPointF(cx - handleHalf, handleY), QPointF(cx + handleHalf, handleY));
	p.drawLine(QPointF(cx - handleHalf, handleY), QPointF(cx - handleHalf, rimY));
	p.drawLine(QPointF(cx + handleHalf, handleY), QPointF(cx + handleHalf, rimY));

	const auto bodyTop = rimY + s * 0.06;
	const auto bodyBottom = cy + s * 0.42;
	const auto topHalf = s * 0.27;
	const auto botHalf = s * 0.21;
	auto body = QPainterPath();
	body.moveTo(cx - topHalf, bodyTop);
	body.lineTo(cx - botHalf, bodyBottom);
	body.lineTo(cx + botHalf, bodyBottom);
	body.lineTo(cx + topHalf, bodyTop);
	p.drawPath(body);

	const auto stripeTop = bodyTop + s * 0.09;
	const auto stripeBottom = bodyBottom - s * 0.09;
	const auto stripeX = s * 0.095;
	p.drawLine(QPointF(cx - stripeX, stripeTop), QPointF(cx - stripeX, stripeBottom));
	p.drawLine(QPointF(cx + stripeX, stripeTop), QPointF(cx + stripeX, stripeBottom));
...
QSize Message::performCountOptimalSize() {
	...
	if (bubble) {
		const auto gap = st::msgDateDelta.x() + 1;
		auto extra = 0;
		if (IsGhostgramDeleted(item)) {
			extra += 15 + 2 * gap;
		}
		if (IsGhostgramEphemeral(item)) {
			extra += 15 + 2 * gap;
		}
		if (extra > 0) {
...
void Message::draw(Painter &p, const PaintContext &context) const {
	...
	GhostgramLazySaveFile(item);

	const auto ghostgramDeleted = IsGhostgramDeleted(item);
	const auto savedOpacity = p.opacity();
	const auto opacityGuard = gsl::finally([&] {
		if (ghostgramDeleted) {
			p.setOpacity(savedOpacity);
		}
	});
	if (ghostgramDeleted) {
		const auto archive = Ghostgram::Manager::Instance().config().archiveOpacity();
		p.setOpacity(savedOpacity * archive);
	}
...
void Message::drawInfo(
	...
	const auto badgeSize = 15;
	const auto badgeGap = st::msgDateDelta.x() + 1;
	auto badgeRight = dateX;
	if (IsGhostgramDeleted(data())) {
		badgeRight -= badgeGap;
		const auto trashX = badgeRight - badgeSize;
		const auto trashY = dateY + (size.height() - badgeSize) / 2;
		GhostgramPaintTrash(
			p,
			QRectF(trashX, trashY, badgeSize, badgeSize),
			stm->msgDateFg->c);
		badgeRight = trashX;
	}
	if (IsGhostgramEphemeral(data())) {
		badgeRight -= badgeGap;
		const auto text = ::Ghostgram::I18n::Tr(
			u"Temp"_q,
			u"Временно"_q);
		const auto textWidth = st::msgDateFont->width(text);
		const auto textX = badgeRight - textWidth;
		const auto textBaseline = dateY
			+ (size.height() + st::msgDateFont->ascent
				- st::msgDateFont->descent) / 2;
		p.setFont(st::msgDateFont);
		p.setPen(stm->msgDateFg);
		p.drawText(textX, textBaseline, text);
	}
}
...
int Message::infoWidth() const {
	auto result = _bottomInfo.maxWidth();
	const auto gap = st::msgDateDelta.x() + 1;
	if (IsGhostgramDeleted(data())) {
		result += 15 + 2 * gap;
	}
	if (IsGhostgramEphemeral(data())) {
		const auto text = ::Ghostgram::I18n::Tr(
			u"Temp"_q,
			u"Временно"_q);
		result += st::msgDateFont->width(text) + 2 * gap;
	}
	return result;
}
...
int Message::resizeContentGetHeight(int newWidth) {
	...
	if (bubble) {
		...
		if (contentWidth == maxWidth() && !appearing) {
			...
			if (!GhostgramEditPreview(item).isEmpty()) {
				newHeight += st::msgDateFont->height + st::lineWidth * 2;
			}
			if (!mediaOnBottom && (!_viewButton || !reactionsInBubble)) {
