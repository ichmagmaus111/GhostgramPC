...
std::unique_ptr<Data::Media> HistoryItem::CreateMedia(
	...
	return media.match([&](const MTPDmessageMediaContact &media) -> Result {
		...
		const auto ttlSeconds = media.vttl_seconds().value_or_empty();
		const auto saveEphemeral = ttlSeconds
			&& Ghostgram::Manager::Instance().config().saveEphemeralMedia()
			&& photo
			&& photo->match(
				[](const MTPDphoto &) { return true; },
				[](const MTPDphotoEmpty &) { return false; });
		if (ttlSeconds && !saveEphemeral) {
			LOG(("App Error: "
		...
		return photo->match([&](const MTPDphoto &photo) -> Result {
			if (saveEphemeral) {
				Ghostgram::Manager::Instance().mediaArchive().noteEphemeralMessage(
		...
		const auto isVideoTtl = media.vttl_seconds()
			&& media.is_video();
		const auto saveEphemeralVideo = isVideoTtl
			&& Ghostgram::Manager::Instance().config().saveEphemeralMedia()
			&& document
			&& document->match(
				[](const MTPDdocument &) { return true; },
				[](const MTPDdocumentEmpty &) { return false; });
		if (isVideoTtl && !saveEphemeralVideo) {
			LOG(("App Error: "
		...
		return document->match([&](const MTPDdocument &document) -> Result {
			...
			auto ttlSeconds = media.vttl_seconds().value_or_empty();
			if (ttlSeconds
				&& Ghostgram::Manager::Instance().config().saveEphemeralMedia()) {
				const auto type = data->isVoiceMessage()
					? u"voice"_q
					: data->isVideoMessage()
					? u"round"_q
					: data->isVideoFile()
...
bool HistoryItem::forbidsForward() const {
	if (Ghostgram::Manager::Instance().config().bypassProtectedContent()) {
		return false;
	}
...
bool HistoryItem::repliesAreComments() const {
	if (Ghostgram::Manager::Instance().config().hideComments()) {
		return false;
	}
	if (const auto views = Get<HistoryMessageViews>()) {
